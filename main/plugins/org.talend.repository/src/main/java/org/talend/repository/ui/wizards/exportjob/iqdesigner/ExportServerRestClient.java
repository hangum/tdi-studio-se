...
public class ExportServerRestClient {
	...
	// public static void authAndFileSend(ExporterServerVO vo) throws Exception
	// {
	// HttpResponse response = Unirest.post(vo.getHost() + vo.getJob_path() +
	// "/upload-job-archive")
	// // 주의: 헤더는 없어야 정상 호출됩니다.
	...
	// .asBinary();
	//
	// System.out.println("===> Status code " + response.getStatus() + ", Status
	// text is " + response.getStatusText());
	...
	// throw new Exception("Status code " + response.getStatus() + ", Status
	// text is " + response.getStatusText());
	// }
	//
